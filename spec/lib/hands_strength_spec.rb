...
RSpec.describe "HandsStrength" do
  ...
  describe "self.poker_api(card)" do
    context "when input is valid" do
      it "returns ストレートフラッシュ" do
        input = "S1 S2 S3 S4 S5"
        expect(PokerHands.poker_api(input)).to eq("ストレートフラッシュ")
      end
      it "returns フラッシュ" do
        input = "S1 S2 S3 S4 S9"
        expect(PokerHands.poker_api(input)).to eq("フラッシュ")
      end
      it "returns ストレート" do
        input = "D1 H2 C3 S4 D5"
        expect(PokerHands.poker_api(input)).to eq("ストレート")
      end
      it "returns フォー・オブ・ア・カインド" do
        input = "S1 D9 H9 C9 S9"
        expect(PokerHands.poker_api(input)).to eq("フォー・オブ・ア・カインド")
      end
      it "returns フルハウス" do
        input = "S1 D1 H9 C9 S9"
        expect(PokerHands.poker_api(input)).to eq("フルハウス")
      end
      it "returns スリー・オブ・ア・カインド" do
        input = "S1 D12 H9 C9 S9"
        expect(PokerHands.poker_api(input)).to eq("スリー・オブ・ア・カインド")
      end
      it "returns ツーペア" do
        input = "S1 D1 H9 C9 S13"
        expect(PokerHands.poker_api(input)).to eq("ツーペア")
      end
      it "returns ワンペア" do
        input = "S1 D1 H11 C5 S8"
        expect(PokerHands.poker_api(input)).to eq("ワンペア")
      end
      it "returns ハイカード" do
        input = "S12 D4 H7 C9 S13"
        expect(PokerHands.poker_api(input)).to eq("ハイカード")
      end
    end
